...
const OrderHistoryFarmer = () => {
    ...
    useEffect(() => {
        const fetchOrderHistory = async () => {
            ...
            try {
                const response = await axios.get(`http://localhost:3000/orders/farmOwner/${customerId}`);
    ...
    const handleAccept = async (item) => {
        console.log(item)
        try {
            const response = await fetch(`http://localhost:3000/orders/${item.orderId}/accept`, {
                method: 'PUT',
                headers: {
            ...
            if (response.ok) {
                const updatedProduct = allProducts.map(product => 
                    product._id === item._id ? { ...product, status: 'accepted' } : product
                );
                setAllProducts(updatedProduct);
                alert(`Item ${item.name} accepted successfully`);
            } else {
                console.error('Failed to accept the item', response.statusText);
                alert('Failed to accept the item');
            }
        } catch (error) {
            console.error('Error accepting the item:', error);
            alert('Error accepting the item');
        }
    };
                                            ...
                                            <div className="flex-1 bg-pink text-white text-center h-10 pt-1" style={{ marginTop: "8%" }}>
                                                {item.status === 'pending' ? (
                                                    <div className="flex flex-col space-y-2">
                                                        <button className="bg-green-500 text-white px-4 py-2 rounded" onClick={() => handleAccept(item)}>
                                                            Accept
                                                        </button>
...
export default OrderHistoryFarmer;
